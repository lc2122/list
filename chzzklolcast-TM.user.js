...
function applyLolcastSidebarControl() {
    ...
    // 세션 내에서 사이드바가 이미 닫혔는지 추적하는 플래그
    let leftSidebarClosed = false;
    let rightSidebarClosed = false;
    ...
    function applySavedSettings() {
        ...
        if (hideLeft && !leftSidebarClosed) {
            waitForElement('#menu-button', (menuButton) => {
                menuButton.click();
                leftSidebarClosed = true; // 닫힘 상태로 설정
            });
        }
        if (hideRight && !rightSidebarClosed) {
            waitForElement('#close-button', (closeButton) => {
                closeButton.click();
                rightSidebarClosed = true; // 닫힘 상태로 설정
            });
        }
    }
    ...
    function checkAndApply() {
        ...
        if (url.includes("/#/player/")) {
            ...
            setTimeout(applySavedSettings, 2000); // 1초 후 사이드바 설정 적용
        }
    }
