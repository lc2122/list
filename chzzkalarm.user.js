...
// @version      1.26
// @description  네이버 치지직 팔로우 방송알림 (페이지 접속 없이 백그라운드 동작, lolcast 링크 사용, 기상 나팔 소리 포함)
...
(async function() {
    ...
    function onairNotificationPopup(data) {
        try {
            ...
            const cooldown = 60 * 1000; // 60초 쿨다운

            // 동일 채널에 대해 쿨다운 내 중복 알림 방지
            if (lastNotificationTimes[channelId] && (now - lastNotificationTimes[channelId] < cooldown)) {
                console.log(`CHIZZK.follow-notification :: Skipping duplicate notification for ${data.channelName} from ${window.location.href}`);
                return;
            }
            ...
            if (settingBrowserNoti) {
                console.log(`CHIZZK.follow-notification :: Triggering notification for ${channelName} from ${window.location.href}`);
                GM_notification({
                    title: channelName,
                ...
                lastNotificationTimes[channelId] = now; // 알림 발생 후 시간 기록
                console.log(`CHIZZK.follow-notification :: Notification triggered successfully for ${channelName} at ${new Date(now).toISOString()}`);
            } else {
